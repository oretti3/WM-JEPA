...
def _write_csv(output_root: Path, metrics_l1, metrics_l2):
    ...
    csv_path = output_root / "tworooms_tdmpc_compare.csv"
    all_keys = sorted(set(metrics_l1) | set(metrics_l2))
    with csv_path.open("w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["metric", "baseline", "tdmpc"])
        for key in all_keys:
...
def main():
    parser = argparse.ArgumentParser(
        description="Run TwoRooms baseline vs TD-MPC-style hierarchy comparison."
    )
    parser.add_argument(
        "--config_l1",
        default="PLDM_hieral/configs/tworooms_l1_tdmpc.yaml",
        help="Path to baseline config",
    )
    parser.add_argument(
        "--config_l2",
        default="PLDM_hieral/configs/tworooms_tdmpc.yaml",
        help="Path to TD-MPC-style hierarchy config",
    )
    ...
    parser.add_argument(
        "--l2_from_scratch",
        action="store_true",
        help="Train TD-MPC hierarchy without loading the latest L1 checkpoint.",
    )
