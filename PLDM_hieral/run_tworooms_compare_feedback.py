...
def _write_csv(output_root: Path, metrics_l1, metrics_l2):
    ...
    csv_path = output_root / "tworooms_feedback_compare.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Run TwoRooms baseline vs feedback hierarchy comparison."
    )
    parser.add_argument(
        "--config_l1",
    ...
    parser.add_argument(
        "--config_l2",
        default="PLDM_hieral/configs/tworooms_feedback.yaml",
        help="Path to feedback hierarchy config",
    )
    ...
    parser.add_argument(
        "--l2_from_scratch",
        action="store_true",
        help="Train feedback hierarchy without loading the latest L1 checkpoint.",
    )
    ...
    if metrics_l1 or metrics_l2:
        print("Metric,Baseline,Feedback")
